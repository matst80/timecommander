...
namespace WebdocOrder
{
    ...
    public class ContactPerson : dbBase
    {
        ...
        {
            return ContactPerson.GetCustomList<ContactPerson>("SELECT * FROM ContactPerson WHERE Name like '%" + p + "%' or Email like '%" + p + "%' or REPLACE(REPLACE(MobilePhone,'-',''),' ','') like '%" + p + "%' or REPLACE(REPLACE(Phone,'-',''),' ','') like '%" + p + "%'");
        }
